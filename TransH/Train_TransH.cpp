        ...
        for (int eval=0; eval<neval; eval++)
        {
        	...
        	res=0;
         	for (int batch = 0; batch<nbatches; batch++)
         	{
                ...
                sprintf(tmp,"%d",eval);
                string s_epoch(tmp);
                FILE* f1 = fopen((data+"_"+version+"/A."+version+s_epoch).c_str(),"w");
                FILE* f2 = fopen((data+"_"+version+"/relation2vec."+version+s_epoch).c_str(),"w");
                FILE* f3 = fopen((data+"_"+version+"/entity2vec."+version+s_epoch).c_str(),"w");
                times+=1;
                for (int i=0; i<relation_num; i++)
                {
                    for (int ii=0; ii<n; ii++)
