    ...
    void bfgs()
    {
        ...
        A_tmp = A;
            for (int epoch=0; epoch<nepoch; epoch++)
            {
                ...
                FILE* f1 = fopen((data+"_"+version+"/relation2vec."+version+s_epoch).c_str(),"w");
                FILE* f2 = fopen((data+"_"+version+"/entity2vec."+version+s_epoch).c_str(),"w");
                FILE* f3 = fopen((data+"_"+version+"/A."+version+s_epoch).c_str(),"w");
                for (int i=0; i<relation_num; i++)
                {
                    for (int ii=0; ii<m; ii++)
...
void prepare()
{
	...
	while (fscanf(f_kb,"%s",buf)==1)
    {
        ...
        //the relation -> s3, entity->s1 or ->s3 if change the dataset these should be changed
